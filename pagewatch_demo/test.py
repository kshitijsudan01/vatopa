...
class processmap(object):
    ...
    # Given start and end addr, this returns the entries from pagemap
    def range(self, startaddr, endaddr):
        off = (startaddr / 4096) * 8
        size = ((endaddr - startaddr) / 4096)

        '''
        print "off = %x,size = %d" % (off,size)
        #dd = self.data[off+16:off+3*8]
        #print "%s" % dd[0]

        #if len(dd):
            #qq = "Q"
            #ll = struct.unpack(qq,dd)
            #array_here = array.array("L", ll)
            #print "array_here[0] = %d" % array_here[0]
            #pg_present = array_here[0] >> 63
            #print "pg_present = %d" % pg_present
            #pg_swapped = ((array_here[0] >> 62) & 1) >> 1
            #print "pg_swapped = %d" % pg_swapped
            # 1F80000000000000 = 2269814212194729984
            #pg_shift = ((array_here[0] & 2269814212194729984) >> 55)
            #pg_size = 1 << pg_shift
            #print "PAGE_SIZE (Bytes) = %d" % pg_size
            # 0x7FFFFFFFFFFFFF = 36028797018963967
            #pfn = (array_here[0] & 36028797018963967)
            #print "PFN = %d" % pfn
            '''
...
def main():
    ...
    print "\nReading /proc/self/maps file"
    for i in range(len(mr)):
        print "%08x-%08x %s" % (mr[i][0],mr[i][1],mr[i][2])

    print "\nreading /proc/self/pagemap file"
    for k in range(len(mr)):
        print "%08x-%08x %s" % (mr[k][0],mr[k][1],mr[k][2])
        arr = map_for_pid.range(mr[k][0],mr[k][1])
