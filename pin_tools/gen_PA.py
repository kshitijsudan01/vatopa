...
# this gets us the instrumented app's PID
pid = os.getpid()

print "pid in python = %d" % pid

for p in range(len(sys.argv)) :
    print "sys.argv[%d] = %s" % (p, sys.argv[p])

trace_file = open(sys.argv[1],'a')
#trace_file.seek(0)
        ...
        for i in range(size) : 
            ...
            if len(d):
                ...
                if pfn != 0 :
                    ...
                    print >> trace_file, "addr = %012x VPN = %012x, PFN = %012x" % (addr,vpn,pfn)
...
def main():
    ...
    for i in range(len(mr)):
        print >> trace_file, "%08x-%08x %s" % (mr[i][0],mr[i][1],mr[i][2])
